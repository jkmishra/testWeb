...
function transform(input, params) {	
	...
	//load("/opt/cms/xsl/wwe/wwe-short-content.js");
...
function processDataForHomePage(input, params) {
	...
	if (input != null) {
		...
		for(var homeIndex in inputList ){ 
		    var homepageData = inputList[homeIndex];
		    if (homepageData.hasOwnProperty('home')||homepageData.hasOwnProperty('short')||homepageData.hasOwnProperty('collection')) {	        		   
    			   // Process all list for home page
        		    var homepageList=null;
        		    if(homepageData.hasOwnProperty('home')){
        		    	homepageList=homepageData.home;
        		    }
        		    if(homepageData.hasOwnProperty('short')){
        		    	homepageList=homepageData.short;
        		    }
        		    if(homepageData.hasOwnProperty('collection')){
        			homepageList=homepageData.collection;
        		    }
        		    // Process all list for home page
            			...
            			var isShortContent = false;
            			var isHomeCollection=false;
	        			...
	        			if (homepageList[i].item.hasOwnProperty("itemTag")) {
	        				...
	        				isShortContent=getHomePageShortContentTag(itemTagArray,homepageList[i].item.itemType['@key']);
	        				isHomeCollection=isCollection(itemTagArray,homepageList[i].item.itemType['@key']);
	        				var title=null;
	        				for ( var platformIndex in itemTagArray) {
		        				...
		        				if (isShortContent && platform=='atv') {
		        				    isResult = true;
		        				    result = getShortContentData(homepageList[i].item,params,title);
		        				}
			        					...
			        					if (isHomeCollection) {
				        				    isResult = true;			        				   
				        				    result=getCollectionData(homepageList[i].item,params);
				        				    result.title = title;
				        				}        			        					
			        					else if (homepageList[i].item.hasOwnProperty("field")) {
		        			    			    	isResult = true;
		        			    				...
		        			    				for ( var j in homepageListFieldArray) {
		        			    					...
		        			    					if (homepageListFieldArray[j]["@primitive"] == 'List' && homepageListFieldArray[j]["@key"] != 'alias-list') {
		        			    						...
		        			    						for ( var j in subListItemArray) {
		        			    							getHomePageData(subListItemArray[j],result,params,j);
		        			    						}
		        			    					}
		        			    				}
		        			    			}
...
function getHomePageData(subListItemArray,result,params,index){
...
//sets flag for short content
function getHomePageShortContentTag(itemTagArray,itemType) {
	var isShortContent;
	var isPlatformTagExists = false;
       for ( var platformIndex in itemTagArray) {
                   // processing only for title=wweOnNow tag on the list
                   if (itemType && itemType=='wwe-short-content' && itemTagArray[platformIndex]["@type"] == 'title'){
                         if(itemTagArray[platformIndex]["@value"] == 'wweOnNow'){
                             isShortContent = true;                         	
                         }
                   }
       }
       return isShortContent;
 }

//sets flag for short content
function isCollection(itemTagArray,itemType) {
	var isHomeCollection;
	var isPlatformTagExists = false;
       for ( var platformIndex in itemTagArray) {
                   // processing only for title=wweOnNow tag on the list
                   if (itemType && itemType=='collection-wwe' && itemTagArray[platformIndex]["@type"] == 'module'){
                         if(itemTagArray[platformIndex]["@value"] == 'home'){
